...
public class Main {
    ...
    public static void main(String[] args){
        encodeDecodeForFont(2);
    }


    public static void encodeDecodeForFont(int fontNum){
        ...
        List<List<Double>> output = new ArrayList<>(font);
        output.addAll(font);

        List<List<Double>> noiseFont = new ArrayList<>(font.size());
        for(List<Double> c : font) {
            noiseFont.add(Noise.saltAndPepper(c, 0.1));
        }
        font.addAll(noiseFont);

        stochasticTraining(autoEncoder, font, output, 200000);
